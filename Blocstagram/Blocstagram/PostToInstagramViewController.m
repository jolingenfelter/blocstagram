...
- (void) addFiltersToQueue {
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (chromeFilter) {
            ...
            [self addCIImageToCollectionView:chromeFilter.outputImage withFilterTitle:NSLocalizedString(@"Chrome",@"CIPhotoEffectChrome")];
        }
    }];
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (instantFilter) {
            ...
            if (gloomFilter) {
                [gloomFilter setValue:result forKey:kCIInputImageKey];
                
                NSNumber *intensity = [NSNumber numberWithFloat:0.6];
                [gloomFilter setValue: intensity forKey:@"inputIntensity"];
                
                NSNumber *radius = [NSNumber numberWithInteger:7];
                [gloomFilter setValue:radius forKey:@"inputRadius"];
            

                result = [gloomFilter.outputImage imageByCroppingToRect:sourceCIImage.extent];
            }
            
            [self addCIImageToCollectionView:result withFilterTitle:NSLocalizedString(@"Sad Dream", @"Sad Dream Filter")];
        }
    }];
...
- (void) sendButtonPressed:(id) sender {
    ...
    if ([[UIApplication sharedApplication] canOpenURL:instagramURL]) {
        ...
        [alertVC addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Cancel", @"cancelbutton") style:UIAlertActionStyleCancel handler:nil]];
